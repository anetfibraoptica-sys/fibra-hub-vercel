...
app.get("/api/mikrotik/profiles", async (req, res) => {
  ...
  try {
    ...
    // Reaproveita a forma de conexão já existente no projeto.
    // A maioria das funções do painel usa conectarMikrotik/obterConexaoMikrotik/RouterOS.
    let rows = [];

    if (typeof conectarMikrotik === "function") {
      const conn = await conectarMikrotik(servidorNome);
      if (conn && typeof conn.write === "function") {
        rows = await conn.write("/ppp/profile/print");
        if (typeof conn.close === "function") {
          try { conn.close(); } catch(e) {}
        }
      }
    } else if (typeof consultarMikrotik === "function") {
      rows = await consultarMikrotik(servidorNome, "/ppp/profile/print");
    } else if (typeof executarComandoMikrotik === "function") {
      rows = await executarComandoMikrotik(servidorNome, "/ppp/profile/print");
    } else if (typeof mikrotikCommand === "function") {
      rows = await mikrotikCommand(servidorNome, "/ppp/profile/print");
    } else {
      throw new Error("Nenhuma função de conexão MikroTik encontrada para buscar profiles.");
    }
    ...
    const profiles = rows
      .map((p) => String(p.name || p.profile || p[".id"] || "").trim())
      .filter(Boolean)
