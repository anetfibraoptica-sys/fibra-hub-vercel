...
/* ============================================================
   CORREÇÃO: MOSTRAR SERVIDORES ONLINE NO PAINEL
   Esta função consulta:
   - /api/status
   - /api/online
   e atualiza cards/listas do dashboard e servidores.
============================================================ */

async function fibraApiGet(path){
  const r = await fetch(path, { cache: "no-store" });
  const txt = await r.text();
  try { return JSON.parse(txt); } catch(e){ return { ok:false, erro: txt || r.statusText }; }
}

function fibraServidorNome(chave){
...
function fibraStatusBadge(ok){
  return ok ? '<span class="badge-online">Online</span>' : '<span class="badge-offline">Offline</span>';
}

function fibraServidorCard(nome, info, totalOnline){
  const ok = !!(info && info.ok);
  const erro = info && info.erro ? info.erro : "";
  const cpu = info && (info.cpu ?? info["cpu-load"] ?? info.cpuLoad);
  const memoria = info && (info.memory ?? info.memoria ?? info.ram);
  const uptime = info && info.uptime;
  const total = totalOnline ?? info?.total ?? 0;

  return `
    <div class="server-card ${ok ? "online" : "offline"}">
      <div class="server-head">
        <div>
          <h3>${fibraEscapeHtml(nome)}</h3>
          <small>${ok ? "Conectado à API MikroTik" : "Sem conexão com a API"}</small>
        </div>
        ${fibraStatusBadge(ok)}
      </div>
      <div class="server-metrics">
        <div><b>${total}</b><span>PPPoE online</span></div>
        <div><b>${cpu !== undefined ? cpu + "%" : "--"}</b><span>CPU</span></div>
        <div><b>${memoria !== undefined ? memoria + "%" : "--"}</b><span>RAM</span></div>
        <div><b>${fibraEscapeHtml(uptime || "--")}</b><span>Uptime</span></div>
      </div>
...
function fibraAtualizarTexto(ids, valor){
  ids.forEach(id => {
    const el = document.getElementById(id);
...
async function carregarServidoresOnlinePainel(){
  const containers = [
    document.getElementById("servidoresOnlineContainer"),
    document.getElementById("statusServidores"),
    document.getElementById("listaServidores"),
    document.getElementById("serversStatus")
  ].filter(Boolean);

  if(!containers.length) return;

  containers.forEach(c => c.innerHTML = '<div class="panel"><p>Carregando servidores...</p></div>');

  let online = null;
  let status = null;

  try { online = await fibraApiGet("/api/online"); } 
  catch(e){ online = { ok:false, erro:e.message, servidores:{} }; }

  try { status = await fibraApiGet("/api/status"); } 
  catch(e){ status = { ok:false, servidores:{} }; }

  const servidoresOnline = online?.servidores || {};
  const servidoresStatus = status?.servidores || status || {};

  const armOnline = servidoresOnline.armando || servidoresOnline.ARMANDO || {};
  const colOnline = servidoresOnline.colonia || servidoresOnline.COLONIA || {};

  const armStatus = servidoresStatus.armando || servidoresStatus.ARMANDO || armOnline || {};
  const colStatus = servidoresStatus.colonia || servidoresStatus.COLONIA || colOnline || {};

  const totalArm = armOnline.total || (armOnline.clientes ? armOnline.clientes.length : 0);
  const totalCol = colOnline.total || (colOnline.clientes ? colOnline.clientes.length : 0);
  const totalGeral = (Number(totalArm) || 0) + (Number(totalCol) || 0);

  fibraAtualizarTexto(["clientesOnlineTotal","totalClientesOnline","cardClientesOnline"], totalGeral);
  fibraAtualizarTexto(["onlineArmando","clientesArmando"], totalArm);
  fibraAtualizarTexto(["onlineColonia","clientesColonia"], totalCol);

  const html = `
    ${fibraServidorCard("Armando Mendes", {...armStatus, ok: armOnline.ok !== undefined ? armOnline.ok : armStatus.ok}, totalArm)}
    ${fibraServidorCard("Colônia Antônio Aleixo", {...colStatus, ok: colOnline.ok !== undefined ? colOnline.ok : colStatus.ok}, totalCol)}
  `;

  containers.forEach(c => c.innerHTML = html);
}

async function carregarClientesOnlineTabelaPainel(){
  const tbody = document.getElementById("tbodyClientesOnline") || document.getElementById("clientesOnlineBody") || document.getElementById("ultimosClientesOnline");
  if(!tbody) return;

  tbody.innerHTML = '<tr><td colspan="6">Carregando clientes online...</td></tr>';

  try{
    const dados = await fibraApiGet("/api/online");
    const lista = dados.clientes || [];
    if(!lista.length){
      tbody.innerHTML = '<tr><td colspan="6">Nenhum cliente online encontrado.</td></tr>';
      return;
    }

    tbody.innerHTML = lista.slice(0, 100).map(c => {
      const cliente = {
        login: c.usuario || c.name || c.login || "",
        nome: c.nome || c.usuario || c.name || "",
        servidor: c.servidor || "",
        pop: c.servidor || "",
        ip: c.ip || c.address || "",
        mac: c.callerId || c.mac || c["caller-id"] || "",
        uptime: c.uptime || "",
        service: c.service || "pppoe",
        raw: c
      };
      ...
      return `
        <tr class="linha-clicavel" onclick='abrirClienteOnline(${JSON.stringify(cliente).replace(/'/g, "&apos;")})'>
          <td>${fibraEscapeHtml(cliente.login)}</td>
          <td>${fibraEscapeHtml(fibraServidorNome(cliente.servidor))}</td>
          <td>${fibraEscapeHtml(cliente.ip)}</td>
          <td>${fibraEscapeHtml(cliente.mac)}</td>
          <td>${fibraEscapeHtml(cliente.uptime)}</td>
          <td>🟢 Online</td>
        </tr>
      `;
    }).join("");
  }catch(e){
    tbody.innerHTML = `<tr><td colspan="6">Erro ao carregar: ${fibraEscapeHtml(e.message)}</td></tr>`;
  }
}

async function fibraInicializarPainelOnline(){
  await carregarServidoresOnlinePainel();
  await carregarClientesOnlineTabelaPainel();
}

// Roda automaticamente quando abrir dashboard/servidores/clientes.
document.addEventListener("DOMContentLoaded", () => {
  setTimeout(() => {
    fibraInicializarPainelOnline();
  }, 300);
});
